...
TEST_F(exercise_decider_test, trivial_noexercise) {
    auto sd = test_data_generator(accelerometer).with_noise(8).constant(1000, Scalar(1000, 1000, 1000));
...
TEST_F(exercise_decider_test, trivial_exercise) {
    auto sd = test_data_generator(accelerometer).with_noise(8).sin(10, 100, Scalar(1000, 1000, 1000));
    auto result = decider.has_exercise(sd);
