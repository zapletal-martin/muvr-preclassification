...
Mat data_patterns::mat(const sensor_data_type type, const uint count, const boost::optional<Scalar> &constant) {
    switch (type) {
        case accelerometer:
        case rotation: {
...
data_pattern_generator data_patterns::sin(const uint8_t period, const Scalar amplitude, const uint noise) {
    return [noise, period, amplitude](const sensor_data_type type, const uint8_t count) {
...
data_pattern_generator data_patterns::constant(const Scalar value, const uint noise) {
    return [noise, value](const sensor_data_type type, const uint8_t count) {
